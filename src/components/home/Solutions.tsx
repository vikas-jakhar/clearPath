    ...
    return (
        <div className='md:py-8 sm:py-4 py-1'>
            <div className="container my-0.5">
                ...
                <div className="lg:mt-10 sm:pt-2 flex flex-wrap justify-center flex-row -mx-3">
                    {SOLUTION_DATA.map((items, idx) => (
                        <div key={idx} className="lg:w-4/12 sm:w-1/2 w-full mt-6 lg:mt-0 px-3">
                            <div className="shadow-4xl flex flex-col items-center bg-off-white transition_slow group hover:bg-deep-blue h-full w-full rounded-xl px-8 pb-[26px]">
                                ...
                                <h2 className={`font-semibold font-messiri text-center transition_slow group-hover:text-off-white mt-7 text-lg sm:text-xl md:text-2xl text-deep-blue ${idx === 1 ? "xl:pr-5" : ""}`}>{items.title}</h2>
